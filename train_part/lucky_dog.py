...
# Data loading params
train_file = "./data_50/prepro_train_50.txt"
valid_file = "./data_50/prepro_valid_50.txt"
test_file = "./data_50/prepro_test_50.txt"
...
best_loss = np.array([[65536.0]], dtype="float32")
start_epoch = 0
...
for i in xrange(num_epochs):
    if(os.path.exists("./epoch_" + str(start_epoch + i+1) + ".pt")):
        if(os.path.exists("./epoch_" + str(start_epoch + i+2) + ".pt")):
            continue
#         strnn_model = torch.load(os.path.join("./", "epoch_" + str(start_epoch + i+1) + ".pt"))
        continue
    # Load Checkpoint
    if(os.path.exists('./checkpoint.pth.tar')):
        checkpoint = torch.load('./checkpoint.pth.tar')
        start_epoch = checkpoint['epoch']
        best_accuracy = checkpoint['best_loss']
        strnn_model.load_state_dict(checkpoint['state_dict'])
        print("=> loaded checkpoint '{}' (trained for {} epochs)".format('./checkpoint.pth.tar', checkpoint['epoch']), flush=True)
...
#         if (j+1) % 2000 == 0:
#             print("batch #{:d}：batch_loss :{}，datetime：{}".format(j+1, total_loss/j, datetime.datetime.now()), flush=True)
    # Evaluation
    if (i+1) % evaluate_every == 0:
        print("==================================================================================", flush=True)
        print("Evaluation at epoch #{:d}: total_loss:{}, datetime:{}".format((i+1), total_loss/j, datetime.datetime.now()), flush=True)
        valid_batches = list(zip(valid_user, valid_td, valid_ld, valid_loc, valid_dst))
        print_score(valid_batches, step=2)
    # Save
    torch.save(strnn_model, os.path.join("./", "epoch_" + str(start_epoch + i+1) + ".pt"))
    # Get bool not ByteTensor
    is_best = bool(total_loss > best_loss)
